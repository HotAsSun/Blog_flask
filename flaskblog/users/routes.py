...
def user_posts(username):
    ...
    posts = Post.query.filter(Post.author == user).order_by(Post.date_posted.desc()).paginate(page=page, per_page=5)
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"your pasword has been reseted your now able to login using the new password","success")
        return redirect(url_for('users.login'))
